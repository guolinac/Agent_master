...
if __name__ == "__main__":
    ...
    被控端
    """
    # 绑定的端口指定了 0.0.0.0，那么通过内网地址或外网地址都可以访问应用
    host = "0.0.0.0"
    port = 9876

    # 拿到命令行的参数
    try:
        # sys.argv是命令行参数从程序外部传值的的一种途径，它是一个列表
        # 列表元素是我们想传进去的的新参数，所以可以用索引sys.argv[]来获得想要的值
        # getopt.getopt()这个函数是为了从外部输入不同的命令行选项时，对应执行不同的功能
        # 目前有短选项和长选项两种格式
        # 短选项格式为"-"加上单个字母选项；长选项为"--"加上一个单词，同样也是程序外部传参
